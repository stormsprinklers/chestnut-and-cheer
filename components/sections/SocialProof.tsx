...
export function SocialProof() {
  return (
    <section className="border-b border-chestnut/10 bg-white py-10">
        ...
        {TRUST_BADGES.map((badge, index) => {
          ...
          return (
            <div key={badge.label} className="flex flex-col items-center text-center">
              ...
              <div className="font-semibold text-chestnut">{badge.label}</div>
              <div className="mt-1 text-sm text-chestnut/60">{badge.detail}</div>
            </div>
          );
        })}
      </div>
